...
def get_train_transforms(cfg: Config):
    ...
    elif aug.type == "gazeclr":
        transforms_ = [
            get_gazeclr_transforms_for_view_1(input_size=aug.input_size, crop_scale=aug.crop_scale,
                                              with_trans_info=with_trans_info),
            get_gazeclr_transforms_for_view_2(input_size=aug.input_size, crop_scale=aug.crop_scale,
                                              with_trans_info=with_trans_info),
    ]
    else:
...
def get_gazeclr_transforms_for_view_1(input_size=INPUT_SIZE, crop_scale=(0.08, 1.0), with_trans_info=False):
    return transforms.Compose([
        transforms.RandomResizedCrop(size=input_size, scale=crop_scale, interpolation=RESIZE_METHOD),
...
def get_gazeclr_transforms_for_view_2(input_size=INPUT_SIZE, crop_scale=(0.08, 1.0), with_trans_info=False):
    return transforms.Compose([
        transforms.RandomResizedCrop(size=input_size, scale=crop_scale, interpolation=RESIZE_METHOD),
